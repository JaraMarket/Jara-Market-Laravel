...
public function validateSignupOTP(RegisterOTPRequest $request)
{
    ...
    $otpRecord = Customer_otp::where('otp', $otp)->where('voterID', $voterID)->where('created_at', '>=', now()->subMinutes(50)) // Only consider OTPs created in the last 15 minutes
    ->first();
    ...
    if ($otpRecord->code == $otp && $otpRecord->created_at >= now()->subMinutes(15)) {
        ...
        $token = $customerData->createToken('Customer_signUp_otp')->plainTextToken;
