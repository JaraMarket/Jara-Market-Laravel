...
public function fetchProfile($email)
    ...
    public function editProfile($email, Request $request)
    {
        ...
        $customerData = Customer::find($email);
        if (!$customerData) {
            return response()->json(['message' => 'Customer Record not found'], 400);
        }

       // $websiteinfo = tblwebsiteinfo::first();
        //$charges = $websiteinfo->base_Fee + $request->txtminute * $websiteinfo->distance_Fee;

        $customerData->update([
            'pickup_address' => $request->txtpickupAddress,
            'pickup_phone' => $request->txtpickupPhone,
            'pickup_state' => $request->txtpickupState,
            'receiver_name' => $request->txtreceiverName,
            'receiver_address' => $request->txtreceiverAddress,
            'receiver_phone' => $request->txtreceiverPhone,
            'receiver_state' => $request->txtreceiverState,
            'receiver_email' => $request->txtreceiverEmail,
            'package_type' => $request->cmdpackageType,
            'package_weight' => $request->txtpackageWeight,
            'package_qty' => $request->txtpackageQty,
            'comment' => $request->txtcomment,
            'charges' => $charges
        ]);

        if ($customerData) {
            return response()->json(['success' => true, 'message' => 'Profile Updated Successfully','data' =>$order], 200);
        } else {
            return response()->json(['success' => false, 'message' => 'Problem Updating Order'], 400);
        }
    }
