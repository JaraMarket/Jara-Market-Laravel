...
public function validateCustomerRegisterOTP(RegisterOTPRequest $request)
{
    ...
    $customerData = Customer_otp::where('customer_id', $customer_id)->first();
    if (!$customerData) {
        return response()->json(['success' => false, 'message' => 'No customer record found'], 404);
    }

    $otpRecord = Customer_otp::where('otp', $otp)->where('email', $customer_id)->where('created_at', '>=', now()->subMinutes(50)) // Only consider OTPs created in the last 15 minutes
    ->first();
