    ...
    public function Customer_Register(customerSignupRequest $request)
{
    ...
    if ($referralCode) {
     ...
     // Retrieve REFERRAL_BONUS from environment
$referralBonus = env('REFERRAL_BONUS');

// Convert REFERRAL_BONUS to float (assuming it's a numeric value)
$referralBonus = (float) $referralBonus;

// Retrieve the current wallet value
$wallet = $referrer->wallet;

// Convert wallet value to float (since it's a varchar field)
$wallet = (float) $wallet;

// Add REFERRAL_BONUS to wallet
$newWallet = $wallet + $referralBonus;

// Update the referrer's wallet
$referrer->update([
    'wallet' => (string) $newWallet, // Convert back to string before updating
]);
        } else {
